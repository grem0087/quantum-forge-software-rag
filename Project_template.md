...
Обновленный бот (с настройками защит) находится [/Task5/rag_bot_secure.py](./Task5/rag_bot_secure.py). Защиты задаются при старте через параметр _security_config_ типа _SecurityConfig_.

А для запуска как и раньше будем использовать  [/Task5/run_bot_secure.py](./Task5/run_bot_secure.py)
...
Для запуска и проверки можно выполнить те же шаги из задания 4, на этот раз все зависимости остались прежними.

1. Загружаем вредоносный документ в базу
```
    python inject_malicious.py
```

Вывод
![py](./Task5/inject_malicious.png)

1. Запускаем бота

```
    python python run_bot_secure.py
```

2. Выключаем защиту, проверяем

![py](./Task5/sec_off.png)

    Выдал все явки пароли

3. Проверка с включенной защитой
...
![py](./Task5/test_success.png)

5 отказов на поиск чувтвительной инф-ии, либо инф-ии о том чего нет:

![py](./Task5/test_denined.png)


Файл отчета сформирован: [test_security_log](./Task5/test_security_log.json)

**Об используемой защите:**

|Слой |	Название |	Что делает|	Когда срабатывает|
| ----|------- | ----------- | ---- |
| 1 | 	Pre-prompt  | 	Системное сообщение запрещает выполнять команды из документов  | 	Всегда (часть промпта)  |
|  2  |   Chunk filter  |  Regex-фильтр отбрасывает чанки с паттернами инъекций  |  До отправки контекста в LLM  |
|  3  | 	Output filter | 	Regex-проверка ответа на утечку паролей/секретов | 	После генерации ответа |

**Ожидаемое поведение:**
 
Без защиты - вредоносный чанк попадает в контекст → LLM может повторить «Суперпароль root: swordfish»

С защитой (все 3 слоя):
 - Слой 1 блокирует вредоносный чанк → он не попадает в контекст
 - Слой 2 даже если чанк проскочил — LLM инструктирован игнорировать команды
 - Слой 3 даже если LLM сгенерировал утечку — ответ будет заблокирован

# Задание 6. Автоматическое ежедневное обновление базы знаний

Скрипт `update_index.py`[py](./Task6/update_index.py) автоматически сканирует папку `data_upload/`,
обрабатывает новые `.md` файлы и добавляет их в векторный индекс ChromaDB. После добавления файлы удаляются. Логи храним в файле с постоянным инкрементом.

Не оч надежно, но для примера кажется достаточно.

### Ручная настройка
- Откройте taskschd.msc (Планировщик заданий)
- Создайте задачу:
- - Имя: UpdateRAGIndex
- - Триггер: ежедневно в 06:00
- - Действие: запуск run_update.bat
- - Вкладка "Параметры":
- - "При сбое перезапускать через: 10 минут"
- - "Число попыток перезапуска: 3"

### Поток данных выглядит следующим образом:

```
data_upload/*.md -> чанкование -> эмбеддинги -> ChromaDB -> удаление файла
```

![py](./Task6/update_index.png)

Логи успешной итерации [update_index_log.txt](./Task6/update_index_log.txt)

# Задание 7. Аналитика покрытия и качества базы знаний

1. Скрипт удаления [remove_entities.py](./Task7/remove_entities.py) удаляет данные о сущностях "Загра Бродмать", "Джим Рэйнор-дурак", "Нова Терра Прекрасная".

2. Скрипт запуска проверки [evaluate.py](./Task7/evaluate.py) по заранее созданным "золотым запросам" [golden_questions.txt](./Task7/golden_questions.txt) формирует лог [logs.csv](./Task7/logs.csv) и файл отчета [evaluation_report.md](./Task7/evaluation_report.md)

Вцелом точность: 6/9 (66.7%). один ответ был неожиданный (хотя температура 0.1), и 2 пропуска - потомучто нет данных в базе.

3. Обработка запроса и оценка качества RAG-бота.

![sequence_diagram.txt](./Task7/sequence_diagram.png)